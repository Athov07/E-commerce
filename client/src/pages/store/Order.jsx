...
import { MapPin, CreditCard, Loader2, ArrowLeft, ShieldCheck, AlertCircle } from "lucide-react";
...
const Order = () => {
  ...
  const getSubtotal = () => {
    ...
    if (cart?.items) {
      return cart.items.reduce((acc, item) => acc + (Number(item.price) * Number(item.quantity)), 0);
    }
    return 0;
  };
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Update global cart count and redirect
  ...
  return (
    <div className="max-w-6xl mx-auto py-10 px-4">
      <button onClick={() => navigate("/address")} className="flex items-center gap-2 text-gray-500 hover:text-blue-600 mb-6 font-medium transition-colors">
        <ArrowLeft size={18} /> Change Address
      </button>

      <h1 className="text-4xl font-black text-gray-900 mb-10">Checkout Summary</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <p className="font-bold text-gray-800 text-lg">{selectedAddress.full_name}</p>
              ...
              <div>
                ...
                <p className="text-gray-600 leading-relaxed text-sm">
                  ...
                  <br />
                  {selectedAddress.city}, {selectedAddress.state} - {selectedAddress.pin_code}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-50 bg-gray-50/50">
              <h3 className="font-bold text-gray-900">Review Items ({cart?.items?.length || 0})</h3>
            </div>
            <div className="divide-y divide-gray-50">
              {cart?.items?.length > 0 ? (
                cart.items.map((item) => (
                  <div key={item.productId || item._id} className="p-6 flex items-center gap-6 group">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400 mt-1">Quantity: <span className="font-bold text-gray-700">{item.quantity}</span></p>
                    </div>
                    <div className="text-right">
                      <p className="font-black text-gray-900">Rs.{Number(item.price) * Number(item.quantity)}</p>
                    </div>
                  </div>
                ))
              ) : (
                <div className="p-10 text-center text-gray-400">No items found in cart.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-xl sticky top-24">
            <h3 className="text-2xl font-black text-gray-900 mb-8">Order Total</h3>
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="flex justify-between text-gray-500 font-medium">
                ...
                <span className={deliveryCharges === 0 ? "text-green-600 font-bold" : "text-gray-900 font-bold"}>
              ...
              <div className="pt-6 border-t-2 border-dashed border-gray-100 flex justify-between items-center">
                ...
                <span className="text-4xl font-black text-blue-600">Rs.{finalTotal}</span>
              </div>
            </div>
            ...
            >
              {isPlacing ? <Loader2 className="animate-spin" /> : <><CreditCard size={24}/> PLACE ORDER</>}
            </button>
            ...
            {subtotal < 500 && subtotal > 0 && (
              <div className="mt-4 flex items-start gap-2 p-3 bg-amber-50 rounded-xl text-amber-700 text-[11px] font-medium border border-amber-100">
                ...
                <span>Add Rs.{500 - subtotal} more to your cart to unlock <b>FREE Delivery</b>!</span>
              </div>
            )}
            ...
            <div className="mt-6 flex items-center justify-center gap-2 text-green-600 bg-green-50 py-3 rounded-xl border border-green-100">
              <ShieldCheck size={18} />
              <span className="text-xs font-bold uppercase tracking-widest">100% Secure Payment</span>
            </div>
          </div>
        </div>
      </div>
