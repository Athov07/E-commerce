...
const Payment = () => {
  ...
  if (!orderId) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => navigate("/")} className="text-blue-600 font-bold">Return Home</button>
      </div>
    );
  }
  ...
  const handleCardSubmit = async (cardData) => {
    setLoading(true);
    try {
      await paymentService.processCardPayment({ orderId, ...cardData, amount: total });
  ...
  return (
    <div className="max-w-xl mx-auto py-10 px-4">
      ...
      <p className="text-gray-500 mb-8">Choose how you'd like to pay for Order #{orderId.slice(-6)}</p>
          ...
          {method === "RAZORPAY" ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-left-4">
              <div className="bg-blue-50 p-6 rounded-3xl border border-blue-100 text-center">
                <p className="text-blue-600 text-xs font-black uppercase tracking-widest mb-2">Secure Gateway</p>
              ...
              />
              <p className="text-center text-xs text-gray-400">Supports UPI, Netbanking, and Wallets</p>
            </div>
          ) : (
            <CardForm onSubmit={handleCardSubmit} loading={loading} total={total} />
          )}
        </div>

        <div className="border-t border-gray-100 pt-8 flex items-center justify-center gap-4 text-gray-400">
          <div className="flex items-center gap-1"><ShieldCheck size={16}/> <span className="text-[10px] font-bold uppercase">SSL Encrypted</span></div>
          <div className="flex items-center gap-1"><CheckCircle2 size={16}/> <span className="text-[10px] font-bold uppercase">PCI Compliant</span></div>
        </div>
      </div>
    </div>
  );
