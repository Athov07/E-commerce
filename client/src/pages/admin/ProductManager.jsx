...
import { Plus, Edit, Trash2, Search, Filter } from "lucide-react"; // Added icons
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Total items: {products.length}</p>
        </div>
              ...
              filteredProducts.map((p) => (
                <tr key={p._id} className="hover:bg-gray-50/50 transition-colors">
                  <td className="px-6 py-4 flex items-center gap-4">
                    <img src={p.main_image} alt="" className="w-12 h-12 rounded-lg object-cover bg-gray-100" />
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-md text-xs font-bold ${p.stock < 10 ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>
                      {p.stock} units
                    </span>
                  </td>
