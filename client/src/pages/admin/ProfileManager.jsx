...
const ProfileManager = () => {
  ...
  const completedProfiles = profiles.filter(p => p.avatar_url && p.fullName).length;
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 tracking-tight">User Administration</h2>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Completed Profiles</p>
              <p className="text-2xl font-black text-gray-900">{completedProfiles}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">User Profile</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Contact Details</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Database Identity</th>
                <th className="px-6 py-4 text-xs font-bold text-gray-400 uppercase">Registration Date</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-50">
              {filteredProfiles.length > 0 ? (
                filteredProfiles.map((profile) => (
                  <tr key={profile._id?.$oid || profile._id} className="hover:bg-blue-50/30 transition-colors group">
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-400 font-medium">Standard Account</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5">
                      <div className="flex flex-col gap-1">
                        <span className="font-mono text-[10px] text-gray-400 bg-gray-50 px-2 py-0.5 rounded border border-gray-100 w-fit">
                      ...
                      <div className="flex flex-col items-end">
                        <div className="flex items-center gap-1.5 text-xs font-bold text-gray-700">
                          ...
                          {profile.createdAt?.$date 
                            ? new Date(profile.createdAt.$date).toLocaleDateString() 
                            : profile.createdAt 
                ...
                <tr>
                  <td colSpan="4" className="px-6 py-20 text-center">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-gray-500 font-medium">No matching profiles found in the system.</p>
                    </div>
                  </td>
                </tr>
              )}
