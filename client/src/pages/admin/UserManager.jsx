...
import { Search, Filter, Trash2, UserPlus, ShieldCheck, Loader2 } from "lucide-react";
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500">View and manage platform permissions.</p>
              </div>
            ...
            {error && (
              <div className="p-4 bg-red-50 border border-red-100 text-red-600 rounded-lg text-sm">{error}</div>
            )}
                  ...
                  {filteredUsers.length > 0 ? (
                    filteredUsers.map((user) => (
                      <tr key={user._id} className="hover:bg-blue-50/30 transition-colors">
                        <td className="px-6 py-4 text-gray-900 font-medium">{user.phone}</td>
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded text-xs font-medium ${user.is_verified ? "text-green-600 bg-green-50" : "text-gray-500 bg-gray-50"}`}>
