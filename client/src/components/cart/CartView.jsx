    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="lg:col-span-2 space-y-4">
        {items.map((item) => (
          <div
            key={item.productId}
            className="flex items-center gap-4 bg-white p-4 rounded-2xl border border-gray-200 shadow-sm"
          >
            {/* PRODUCT IMAGE */}
            ...
            <div className="flex-1">
              ...
              <p className={`text-[10px] font-bold uppercase mt-1 ${item.stock === null || item.stock <= 0 ? 'text-red-500' : 'text-gray-400'}`}>
    {item.stockMessage}
  </p>
            </div>
            ...
            <div className="flex items-center gap-3 bg-gray-50 p-1 rounded-xl border">
              ...
              <button
                onClick={() => onUpdateCount(item.productId, item.quantity + 1)}
                className="p-1 hover:bg-white rounded-md transition-all text-gray-600 disabled:opacity-30 disabled:cursor-not-allowed"
